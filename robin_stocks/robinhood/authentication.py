...
def login_step_one(username: str, password: str, by_sms=True, expiresIn=86400):
    """
    Step 1: Initiates the login process.
    Returns a response with either an access token or a challenge that requires further action.
    """
...
def login_step_two_challenge(challenge_id: str, sms_code: str):
    """
    Step 2 (Challenge): Handles the challenge by submitting the SMS code.
    """
    url = challenge_url(challenge_id)
    payload = {"response": sms_code}
    ...
    if "challenge" in res and res["challenge"]["remaining_attempts"] > 0:
        return {
            "success": False,
            "remaining_attempts": res["challenge"]["remaining_attempts"],
        }

    if res.get("status") == "validated":
        return {"success": True}

    raise Exception(f"Challenge failed: {res}")


def login_step_two_mfa(username: str, password: str, mfa_code: str, device_token: str):
    """
    Step 2 (MFA): Completes the login process using MFA.
    """
    payload = {
        "client_id": "c82SH0WZOsabOXGP2sxqcj34FxkvfnWRZBKlBjFS",
        "expires_in": 86400,
    ...
    raise Exception(f"MFA failed: {data.get('detail', 'Unknown error')}")
